...
##    Normalization and dissolve
    Carbon_sequestration_array = Normalization(np.array([[float(x) for x in y.split()] for y in Carbon_sequestration_array[6:]])+Mean_PC,
    ...
    Carbon_sequestration_1 = Carbon_sequestration_array-(Mean_PC/Max_SOCP)
    Carbon_sequestration_2 = Carbon_sequestration_array
    Carbon_sequestration_3 = (Carbon_sequestration_array-(Mean_PC/Max_SOCP))*0.3
    weights = {'F':0.3,'E':0.3,'C':0.3,'S':0.1}

    land_array = dissolve(land_array,10)

    focuszone = [0,len(land_array),0,len(land_array[0])]
##    focuszone = [150,200,100,150]
##    focuszone = [50,100,50,100]
    ...
    max_t = 100
    num_particle = 30
    init,congitive,social = 0.1,0.5,0.5
